# Hi there, I'm Rupesh Shivsharan 👋

<p align="left">
  <a href="mailto:shivsharanrupesh1984@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" /></a>
  <a href="[https://www.linkedin.com/in/rupesh-shivsharan](https://www.linkedin.com/in/rupesh-shivsharan-623153141/)"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
  <img src="https://img.shields.io/badge/Location-Mississauga%2C%20ON%2C%20Canada-green?style=for-the-badge&logo=googlemaps&logoColor=white" />
</p>

---

## 🧠 About Me

I'm a **Senior AI Architect & Technical Lead** with 15+ years of experience building production-grade AI systems for regulated industries. Currently at **Genpact – Headstrong Canada**, I design and deliver enterprise Agentic AI, RAG pipelines, and LLM-powered applications for financial services clients.

My work sits at the intersection of **Generative AI**, **Cybersecurity**, and **MLOps** — from multi-agent financial crime investigation pipelines to AI governance frameworks aligned with OSFI, PIPEDA, and FINTRAC.

Previously a **Sr. Principal Consultant at Oracle Canada** (8 years), where I led AI/ML initiatives spanning AML fraud detection, customer churn prediction, and document intelligence.

---

## 🚀 What I Build

- 🤖 **Agentic AI Systems** — Multi-agent pipelines using CrewAI, LangGraph, and Agno with planner–executor and supervisor patterns
- 🔍 **RAG Pipelines** — End-to-end retrieval architectures: OCR → chunking → embedding → vector search → reranking → grounded LLM response
- 🏦 **Financial Crime AI** — AML alert triage, KYC document processing, and automated case management (80% reduction in manual review)
- 🛡️ **AI Safety & Governance** — Guardrails, PII masking, policy-based filtering, audit-ready logging, and responsible AI controls
- 📊 **ML Models** — Churn prediction, customer segmentation (K-Means), threshold optimization, and fraud scoring with XGBoost/LightGBM

---

## 🛠️ Tech Stack

### AI / GenAI
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat-square&logo=langchain&logoColor=white)
![CrewAI](https://img.shields.io/badge/CrewAI-FF6B35?style=flat-square)
![LangGraph](https://img.shields.io/badge/LangGraph-4A90D9?style=flat-square)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat-square&logo=openai&logoColor=white)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=flat-square&logo=huggingface&logoColor=black)
![MCP](https://img.shields.io/badge/MCP-Model%20Context%20Protocol-blueviolet?style=flat-square)

### ML & Data
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=flat-square&logo=scikitlearn&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat-square&logo=tensorflow&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![PySpark](https://img.shields.io/badge/PySpark-E25A1C?style=flat-square&logo=apachespark&logoColor=white)

### Cloud & MLOps
![Azure](https://img.shields.io/badge/Azure%20AI%20Foundry-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white)
![OCI](https://img.shields.io/badge/Oracle%20Cloud-F80000?style=flat-square&logo=oracle&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat-square&logo=kubernetes&logoColor=white)

### Databases & Search
![ChromaDB](https://img.shields.io/badge/ChromaDB-Vector%20DB-9B59B6?style=flat-square)
![Oracle DB](https://img.shields.io/badge/Oracle%20DB-F80000?style=flat-square&logo=oracle&logoColor=white)
![Databricks](https://img.shields.io/badge/Databricks-FF3621?style=flat-square&logo=databricks&logoColor=white)

### Security
![Kali Linux](https://img.shields.io/badge/Kali%20Linux-557C94?style=flat-square&logo=kalilinux&logoColor=white)
![OWASP](https://img.shields.io/badge/OWASP%20LLM%20Top%2010-000000?style=flat-square)
![MITRE ATT&CK](https://img.shields.io/badge/MITRE%20ATT%26CK-CC0000?style=flat-square)

---

## 📌 Featured Projects

### 🕵️ Agentic AI Financial Crime Investigation Pipeline
> Production-grade AML alert triage and case management system using CrewAI, GPT-4, and Oracle KYC retrieval.
- 80% reduction in manual review workload
- Auto-closure of up to 70% of alerts
- 40% fewer false positives vs. legacy rule-based systems
- 5x faster alert-to-case closure time

### 📉 Churn Prediction & Retention Pipeline
> Multi-agent system for retail client churn prediction, persona segmentation, and personalized campaign orchestration.
- 60% reduction in manual analysis time
- 15% increase in customer retention
- ~$1.2M annually in prevented revenue loss

### 💬 Internal IT Support Chatbot (RAG + Slack)
> LangChain + ChromaDB + Cohere-powered enterprise chatbot integrated with Slack for real-time IT support.
- 65% reduction in IT support tickets
- 70% of queries resolved instantly
- Response time reduced from 30+ minutes to seconds

---

## 🎓 Education & Certifications

- 🔐 **Certificate in Penetration Testing** — York University, School of Continuing Studies *( June 2026)*
- 🎓 **MSc Information Systems Management** — De Montfort University, UK *(2011)*
- 🎓 **BE Computer Engineering** — University of Pune, India *(2010)*
- ✅ **ITIL V3 Foundation** Certified

---

## 🏆 Achievements

- 🥇 **Dream Team Award** — Q1 FY2025, Oracle
- 🌟 **Circle of Excellence FY2020** — Oracle (Led Oracle license sale at ING Bank, Amsterdam)
- 🏅 **Best Team Award** — April 2020, Oracle
- 🎖️ **SunGard Kudos** — 2013 & 2015

---

## 🌐 Beyond the Code

When I'm not architecting AI systems, you'll find me on the road on my **2026 Harley-Davidson Nightster Special** 🏍️, exploring Ontario's best routes. 
---

<p align="center">
  <i>"Building AI that's not just intelligent — but trustworthy, auditable, and production-ready."</i>
</p>

